...
test("selected uncached messages are submitted once and imported by custom id", async () => {
  ...
  try {
    ...
    const grouped = groupMessages([
      message(1),
      { ...message(1), studentNumber: "202", child: "Other child", messageId: 99 },
    ]);
    assert.deepEqual(await batches.submit(grouped), { batchId: "batch-1", submitted: 1 });
    assert.equal(created.length, 1);
    assert.deepEqual(createOptions, [{ maxRetries: 0 }]);
    const request = created[0] as { requests: Array<{ custom_id: string; params: {
    ...
    assert.match(request.requests[0]?.params.messages[0]?.content ?? "", /Child & Other child/);
    assert.equal(await batches.submit(grouped), null, "pending messages must not be charged twice");
    ...
    assert.equal(await restartedBatches.submit(grouped), null, "pending state must survive a restart");
    await restartedBatches.refresh();

    assert.equal(restartedAnalyzer.cached(grouped[0]!)?.hasOtherContent, true);
    assert.equal(restartedBatches.statuses()[0]?.status, "ended");
    assert.equal(restartedBatches.statuses()[0]?.imported, 1);
    assert.equal(await restartedBatches.submit(grouped), null, "cached messages must not be charged twice");
  } finally {
...
test("a merged message reuses analysis cached under any affected child", async () => {
  const dir = mkdtempSync(join(tmpdir(), "family-wilma-batch-merged-cache-"));
  let createCalls = 0;
  const fakeAnthropic = {
    messages: { batches: { create: async () => { createCalls += 1; return { id: "unexpected" }; } } },
  } as unknown as Anthropic;
  ...
  try {
    ...
    const analyzer = new MessageAnalyzer("test", store, fakeAnthropic);
    const batches = new AnalysisBatchService(store, analyzer, fakeAnthropic);
    const canonical = message(1);
    const other = { ...message(1), studentNumber: "202", child: "Other child", messageId: 99 };
    store.put(analysisIdentity(other), { calendarItems: [], hasOtherContent: true });
    const grouped = groupMessages([canonical, other]);

    assert.equal(analyzer.cached(grouped[0]!)?.hasOtherContent, true);
    assert.equal(store.get(analysisIdentity(grouped[0]!))?.hasOtherContent, true, "alias cache should be promoted to the logical identity");
    assert.equal(await batches.submit(grouped), null);
...
test("a pending analysis for any merged child prevents a duplicate submission", async () => {
  const dir = mkdtempSync(join(tmpdir(), "family-wilma-batch-merged-pending-"));
  let createCalls = 0;
  const fakeAnthropic = {
    messages: { batches: { create: async () => { createCalls += 1; return { id: "unexpected" }; } } },
  } as unknown as Anthropic;
  ...
  try {
    ...
    const canonical = message(1);
    const other = { ...message(1), studentNumber: "202", child: "Other child", messageId: 99 };
    store.reserveBatch("existing", [{ customId: "message_0", identity: analysisIdentity(other) }]);
    store.attachProviderBatch("existing", "provider-existing");

    assert.equal(await batches.submit(groupMessages([canonical, other])), null);
    assert.equal(createCalls, 0);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
