...
export function renderHomeworkContent(options: {
  ...
  const cards = [
    ...options.pedanet.map((item, index) => ({
      date: item.date,
      sourceOrder: 0,
      html: `<article class="card homework-card source-card"><div><span class="pill">Einari · Peda.net</span></div><h2>${escapeHtml(item.heading)}</h2><div class="homework-body">${escapeHtml(item.content)}</div>${index === 0 ? `<p class="muted homework-meta">Luokan sivulla voi olla vaihtoehtoisia tehtäviä eri ryhmille. <a class="toplink" href="${escapeHtml(item.sourceUrl)}" rel="noopener noreferrer">Avaa lähde</a></p>` : ""}</article>`,
    })),
    ...options.homework.map((item) => ({
      date: item.date,
      sourceOrder: 1,
      html: `<article class="card homework-card"><div><span class="pill">${escapeHtml(item.child)}</span></div><h2>${escapeHtml(item.subject || item.subjectCode || "Kotitehtävä")}</h2><p class="muted homework-meta">${escapeHtml(formatDate(item.date))}${item.teacher ? ` · ${escapeHtml(item.teacher)}` : ""}</p><div class="homework-body">${escapeHtml(item.homework)}</div></article>`,
    })),
