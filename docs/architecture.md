...
**A small SQLite file holds the analysis cache plus the latest successful 30-day message and
homework snapshots.** These replaceable snapshots avoid a slow Wilma refetch after every process
restart. Opening either view starts a background refresh only when its successful snapshot is at
least 15 minutes old; an explicit refresh bypasses that window. Cache rows are scoped by
configuration fingerprints so a changed household or class-page source cannot display the
previous source's data.
